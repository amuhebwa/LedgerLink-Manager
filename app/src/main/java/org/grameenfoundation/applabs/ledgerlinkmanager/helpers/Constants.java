package org.grameenfoundation.applabs.ledgerlinkmanager.helpers;

// End point methods for makes http requests
public class Constants {
    public String SINGLEVSLADETAILS = "getSingleVslaDetails";
    public String validateTrainer = "validateTrainer";
    public String SEARCHVSLA = "searchForVsla";
    public String DEFAULTURL = "http://192.168.1.157/DigitizingDataWebServices/DigitizingDataRestfulWebService.svc/";
}
